...
TEMPLATE = '문서 내용을 바탕으로 질문에 답변해주세요.\n문서 내용:\n{context}\n\n질문:\n{query}\n\n답변:\n'
SUMMARY_TEMPLATE = '문서 내용을 바탕으로 요약해주세요.\n문서 내용:\n{context}\n\n요약:\n'
...
def load_csv(csv_path):
    ...
    supply_map = {
        ...
        'oil_filter': '오일 필터',
        'air_filter': '에어 필터',
        'cabin_air_filter': '에어컨 필터',
        'front_brake_pads': '앞 브레이크 패드',
        'rear_brake_pads': '뒤 브레이크 패드',
        'coolant': '냉각수',
        'wiper_blades': '와이퍼 블레이드',
        'spark_plugs': '스파크 플러그',
        'tires': '타이어',
        'battery': '배터리',
        'transmission_fluid': '미션 오일'
    }
    ...
    for _, row in df.iterrows():
        text = (
            f'내 차의 소모품: {supply_map[row["items"]]}\n'
            f'내 차의 해당 소모품을 사용하여 주행한 거리: {row["mileage_km"]} km\n'
            f'내 차의 해당 소모품을 사용한 개월 수: {row["mileage_month"]}개월\n'
            f'해당 소모품을 교체해야 하는 주행거리: {row["replacement_km"]} km\n'
            f'해당 소모품을 교체해야 하는 개월 수: {row["relacement_month"]}개월\n'
            f'내 차의 해당 소모품의 사용량: {row["used_percent"]}%\n'
            f'내 차의 해당 소모품의 교체 시기: {status_map[row["status"]]}'
        )
        texts.append(text)
...
def load_pdf(pdf_path):
    context = fitz.open(pdf_path)
    texts = []
    page_images_map = {}

    os.makedirs(IMAGE_DIR, exist_ok=True)

    for i, page in enumerate(context):
        ...
        for img_index, img in enumerate(page.get_images(full=True)):
            xref = img[0]
            base_image = context.extract_image(xref)
...
def remove_duplicates_in_docs(docs):
    source_docs = {}
    for d in docs:
...
def generate_llm_answer(query, text_gen, retriever):
    ...
    long_answer = answers['result'].split('Helpful Answer:')[-1].split('Answer:')[0].strip()
    docs = answers.get('source_documents', [])
    source_docs = remove_duplicates_in_docs(docs)
    context = [doc.page_content for doc in source_docs]
    return long_answer, source_docs, context
...
def generate_pdf_answer(query, vectorstore, text_gen):
    ...
    return short_answer, source_docs, context

def generate_summary(context, text_gen):
...
def build_graph(page, vectorstore, text_gen, retriever):
    ...
    def llm_answer(state):
        with st.spinner('LLM 답변 생성 중...'):
            ...
            answer, source_docs, context = generate_llm_answer(query, text_gen, retriever)
        return {**state, 'answer': answer, 'source_docs': source_docs, 'context': context}

    def pdf_answer(state):
        with st.spinner('ChromaDB 검색으로 답변 생성 중...'):
            query = state.get('query', '')
            answer, source_docs, context = generate_pdf_answer(query, vectorstore, text_gen)
        return {**state, 'answer': answer, 'source_docs': source_docs, 'context': context}
    ...
    def get_summary(state):
        ...
        summary = generate_summary(context, text_gen)
...
# UI
# ----------------------
def display_answer(answer, summary, context, images):
...
def get_content(output, page_images_map):
...
def main():
    ...
    if st.sidebar.button('차량 점검하기'):
        with st.spinner('LoRA fine-tuning 기반 차량 점검 중...'):
            ...
            model = lora_fine_tuning(base_model, tokenizer, chunks)
            st.write('차량 점검 완료')
    ...
    for message in st.session_state.messages:
        if message['role'] == 'user':
            ...
            answer, summary, context, images = get_content(output, page_images_map)
            display_answer(answer, summary, context, images)
